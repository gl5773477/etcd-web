...
//Receive receive msg for logChan
func (sh *StreamHandler) Receive(r *Record) {
	...
	default:
		...
		log.Println("ERROR logChan is full, abandon msg:%s, err: %s", buf, err)
	}
}
...
func (sh *StreamHandler) Signal(signal *LogSignal) {
	...
	default:
		log.Println("ERROR sigChan is full, abandon signal Action:%d, Payload:%v", signal.Action, signal.Payload)
	}
}
